...
class UpDataAnime extends Controller
{
    public function upDataAnime(Request $request)
    {
        try {
            $name_anime = $request->input('name-anime'); 
            $name_anime_other = $request->input('name-anime-other');
            $genre = $request->input('genre');  
            $description = $request->input('description');  
            $studio = $request->input('studio');
            $release_date = $request->input('release_date');  
            $episode_a = $request->input('episode_a');  
            $episode_t = $request->input('episode_t'); 
            $status = $request->input('status'); 

            $request->validate([
                'image-anime' => 'required|image|mimes:jpeg,png,jpg,gif|max:2048', 
            ]);
            // Upload ảnh
            if ($request->hasFile('image-anime')) {
                $image_anime = $request->file('image-anime');
                $imageName = time() . '_' . $image_anime->getClientOriginalName();
                $path = 'uploads/images-anime/';
                $image_anime->move($path, $imageName); 
            }

        
            //Tạo mảng để cảnh cáo
            $requiredFields = [
                'name-anime' => 'Tên Anime',
                'name-anime-other' => 'Tên gọi khác',
                'genre' => 'Thể loại',
                'description' => 'Mô tả',
                'studio' => 'Studio',
                'release_date' => 'Ngày phát hành',
                'episode_a' => 'Số tập',
                'episode_t' => 'Thời lượng tập',
                'status' => 'Trạng thái'
            ];
            
            // Duyệt qua mảng và kiểm tra các trường bị bỏ trống
            foreach ($requiredFields as $field => $fieldName) {
                if (!$request->has($field) || $request->input($field) === null || trim($request->input($field)) === '') {
                    return redirect()->back()->with('error', 'Vui lòng điền đầy đủ các trường bắt buộc');
                }
            }

            // Kế thừa nội dung blade
            $bladeContent = <<<EOT
                <!DOCTYPE html>
                <html>
                <head>
                    ...
                    <title>Xem phim {$name_anime}</title>
                                        ...
                                        <div class="row">
                                            <div class="col-6 movie-detail">
                                                <h1 class="movie-title">
                                                    <span class="title-1" itemprop="name">$name_anime</span>
                                                    <span class="title-2" itemprop="name">$name_anime_other</span>
                                                </h1>
                                                <div class="movie-meta-info">
                                                    <dl class="movie-dl">
                                                        <dt class="movie-dt">Số tập: </dt>
                                                        <dd class="movie-dd"><a href="">$episode_a</a></dd><br>
                                                        <dt class="movie-dt">Thể loại: </dt>
                                                        <dd class="movie-dd dd-cat"><a href="?">$genre</a></dd><br>
                                                        <dt class="movie-dt">Nhà sản xuất: </dt>
                                                        <dd class="movie-dd"><a href="danh-sach/tv-series/1.html">$studio</a></dd><br>
                                                        <dt class="movie-dt">Ngày phát hành: </dt>
                                                        <dd class="movie-dd"><a href="?">$release_date</a></dd><br>
                                                        <dt class="movie-dt">Trạng thái</dt>
                                                        <dd class="movie-dd imdb">$status</dd><br>
                                                        </dd><br>
                                            ...
                                            <div class="col-6 movie-image">
                                                <div class="movie-l-img"><img itemprop="image" alt="" src="$path$imageName" id="style-lOE6n" class="style-lOE6n">
            ...
            $bladeFileName = strtolower(str_replace(' ', '_', $name_anime)) . '.blade.php';
            $bladeFileNameNotDotBladePhp = strtolower(str_replace(' ', '_', $name_anime));

            // Save Blade file
            $bladeFilePath = resource_path("views/ANIME/FolderDeCu/{$bladeFileName}");
            file_put_contents($bladeFilePath, $bladeContent);

            //Tạo đường dẫn
            $link = "ANIME.FolderDeCu." . $bladeFileNameNotDotBladePhp;
            

            // Thêm dữ liệu vào cơ sở dữ liệu
            DB::table('films')->insert([
                'Hinh_Anh_Anime' => $path.$imageName,
                'Ten_Anime' => $name_anime,
                'Ten_Khac' => $name_anime_other,
                'The_loai' => $genre,
                'Mo_Ta' => $description,
                'Studio' => $studio,
                'Ngay_Phat_Hanh' => $release_date,
                'So_Tap' => $episode_a,
                'Thoi_Luong_Moi_Tap' => $episode_t,
                'Trang_Thai' => $status,
                'Duong_Dan' =>  $link,
            ]);

            return redirect()->back()->with('success', 'Đã thêm Anime thành công');
        } catch (Exception $e) {
            return redirect()->back()->with('error', 'Đã có lỗi khi up dữ liệu' . $e->getMessage());
        }
        
    }
    public function update(Request $request, $id) {
        try {
            // Lấy anime từ database
            $anime = DataAnime::find($id);
            ...
            // Validate dữ liệu
            $request->validate([
                'image-anime' => 'nullable|image|mimes:jpeg,png,jpg,gif|max:2048',
            ]);

            // Cập nhật thông tin Anime
            $anime->Ten_Anime = $request->input('name-anime'); 
            $anime->Ten_Khac = $request->input('name-anime-other');
            $anime->The_loai = $request->input('genre');  
            $anime->Mo_Ta = $request->input('description');  
            $anime->Studio = $request->input('studio');
            $anime->Ngay_Phat_Hanh = $request->input('release_date');  
            $anime->So_Tap = $request->input('episode_a');  
            $anime->Thoi_Luong_Moi_Tap = $request->input('episode_t'); 
            $anime->Trang_Thai = $request->input('status'); 
            ...
            if ($request->hasFile('image-anime')) {
                ...
                $imageName = time() . '_' . $image_anime->getClientOriginalName();
                $path = 'uploads/images-anime/';
                $image_anime->move(public_path($path), $imageName);

                // Cập nhật đường dẫn hình ảnh
                $anime->Hinh_Anh_Anime = $path . $imageName;
            }
